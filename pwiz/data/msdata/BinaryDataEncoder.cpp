...
namespace pwiz
{
    namespace msdata
    {
        ...
        public:
            ...
            void encode(const vector<T>& data, string& result, size_t* binaryByteCount);
            ...
            template <typename T, typename T32>
            void decode(const string& encodedData, pwiz::util::BinaryData<T>& result)
            {
                                             ...
                                             size_t* binaryByteCount)
        {
            ...
            switch (config_.currentDataType)
            {
                case BinaryDataEncoder::BinaryDataType::Type_MZ:
                    switch (config_.mzTruncationMode)
                    {
                        ...
                        case Trunc_Relative:
                            ...
                            break;
                        default:
                            throw runtime_error("[BinaryDataEncoder::encode()] unknown m/z truncation mode");
                            break;
                    }
                    break;
                case BinaryDataEncoder::BinaryDataType::Type_Intensity:
                    switch (config_.intTruncationMode)
                    {
                        ...
                        case Trunc_Relative:
                            ...
                            break;
                        default:
                            throw runtime_error("[BinaryDataEncoder::encode()] unknown intensity truncation mode");
                            break;
                    }
                    break;
                default: break;
            ...
            if (Numpress_None != config_.numpress)
            {
                // lossy numerical representation
                try
                {
                    ...
                    case Numpress_Linear:
                        ...
                        {
                            double fp = MSNumpress::optimalLinearFixedPointMass(
                                data, dataSize, config_.numpressLinearAbsMassAcc);
                            if (fp < 0.0)
                            {
                                // failure: cannot achieve that accuracy, thus don't numpress (see below)
                                n = 0;
                                byteCount = MSNumpress::encodeLinear(data, dataSize, &numpressed[0],
                                                                     config_.numpressFixedPoint);
                            }
                            else
                            {
                        ...
                        if ((numpressErrorTolerance = config_.numpressLinearErrorTolerance) > 0)
                            // decompress to check accuracy loss
                    ...
                    case Numpress_Slof:
                        ...
                        numpressed.resize(byteCount);
                        if ((numpressErrorTolerance = config_.numpressSlofErrorTolerance) > 0)
                            // decompress to check accuracy loss
            ...
            if (Numpress_None == config_.numpress)
            {
                //  may need 64->32bit conversion and byte ordering
        ...
        void BinaryDataEncoder::Impl::encode(const std::int64_t* data, size_t dataSize, std::string& result,
                                             size_t* binaryByteCount)
        {
    ...
    bool mustEndianize = (config_.byteOrder == ByteOrder_LittleEndian);
        ...
        void BinaryDataEncoder::Impl::decode(const char* encodedData, size_t length,
        ...
        void BinaryDataEncoder::Impl::decode(const char* encodedData, size_t length,
                                             pwiz::util::BinaryData<std::int64_t>& result)
        {
            typedef std::int64_t T;
            typedef std::int32_t T32;
    ...
    bool mustEndianize = (config_.byteOrder == ByteOrder_LittleEndian);
        ...
        PWIZ_API_DECL void BinaryDataEncoder::encode(const std::vector<double>& data, std::string& result,
                                                     size_t* binaryByteCount /*= NULL*/) const
        {
            impl_->encode<double, float>(data, result, binaryByteCount);
        }


        PWIZ_API_DECL void BinaryDataEncoder::encode(const double* data, size_t dataSize, std::string& result,
                                                     size_t* binaryByteCount /*= NULL*/) const
        {
        ...
        PWIZ_API_DECL void BinaryDataEncoder::decode(const char* encodedData, size_t len,
                                                     pwiz::util::BinaryData<double>& result) const
        {
            impl_->decode(encodedData, len, result);
        }

        PWIZ_API_DECL void BinaryDataEncoder::encode(const std::vector<std::int64_t>& data, std::string& result,
                                                     size_t* binaryByteCount /*= NULL*/) const
        {
        ...
        PWIZ_API_DECL void BinaryDataEncoder::encode(const std::int64_t* data, size_t dataSize, std::string& result,
                                                     size_t* binaryByteCount /*= NULL*/) const
        {
            impl_->encode(data, dataSize, result, binaryByteCount);
        }
        ...
        PWIZ_API_DECL void BinaryDataEncoder::decode(const char* encodedData, size_t len,
                                                     pwiz::util::BinaryData<std::int64_t>& result) const
        {
            impl_->decode(encodedData, len, result);
        }

        PWIZ_API_DECL const BinaryDataEncoder::Config& BinaryDataEncoder::getConfig() const
        // get the config actually used - may differ from input for numpress use
        {
        ...
        PWIZ_API_DECL ostream& operator<<(ostream& os, const BinaryDataEncoder::Config& config)
        {
            ...
            os << endl << (config.byteOrder == BinaryDataEncoder::ByteOrder_LittleEndian
                               ? "ByteOrder_LittleEndian"
                               : "ByteOrder_BigEndian") << endl;
            return os;
        }
    } // namespace msdata
} // namespace pwiz
